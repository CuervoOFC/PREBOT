...
if (!args[0] || !text) throw `*${usedPrefix + command} Billie Eilish - Bellyache*`;
const yt_play = await search(args.join(' '));
const ytplay2 = await yts(text);
try {

if (command === 'play.1') {
conn.reply(m.chat, 'DESCARGA HAJIME', m, { contextInfo: { externalAdReply: { mediaUrl: null, mediaType: 1, description: null, title: packname, body: author, previewType: 0, thumbnail: fglog, sourceUrl: fgcanal }}});
try {
...
await conn.reply(m.chat, `#report ${usedPrefix + command}`, m, rcanal);
}}}}}
...
let { data } = await res.json();
await conn.sendFile(m.chat, data.dl, 'error.mp4', `${packname}`, m)
} catch (e1) {
try {  
const axeelUrl = `https://axeel.my.id/api/download/audio?url=${yt_play[0].url}`;
const axeelResponse = await fetch(axeelUrl);
const axeelData = await axeelResponse.json();
if (!axeelData || !axeelData.downloads?.url) throw new Error();
await conn.sendFile(m.chat, axeelData.downloads.url, 'error.mp4', `${packname}`, m)
} catch {
try {
const ryzenUrl = `https://api.ryzendesu.vip/api/downloader/ytmp3?url=${yt_play[0].url}`;
const ryzenResponse = await fetch(ryzenUrl);
...
if (ryzenData.status === 'tunnel' && ryzenData.url) {
const downloadUrl = ryzenData.url;
await conn.sendFile(m.chat, downloadUrl, 'error.mp4', `${packname}`, m)
}
} catch {
try {          
let d2 = await fetch(`https://exonity.tech/api/ytdlp2-faster?apikey=adminsepuh&url=${yt_play[0].url}`);
...
const audiop = await getBuffer(dp.result.media.mp3);
const fileSize = await getFileSize(dp.result.media.mp3);
await conn.sendFile(m.chat, audiop, 'error.mp4', `${packname}`, m)
} catch (e) {
console.error('Error al obtener el audio:', e);
await conn.reply(m.chat, `#report ${usedPrefix + command}`, m);
}}}}}

} catch (e) {
await conn.reply(m.chat, `Intenta mas tarde ${usedPrefix + command}`, m, rcanal);
