const palabras = [
  ...
  "WhatsApp", "Instagram", "tiktok", "presidente", "bot", "películas", "hajime", "hajimebot"
];
...
function juegoTerminado(sender, mensaje, palabra, letrasAdivinadas, intentos) {
  ...
  if (!mensaje.includes("_")) {
    ...
    global.db.data.users[sender].coin += expGanada;
    gam.delete(sender);
...
if (gam.has(m.sender)) return conn.reply(m.chat, "⚠️ Ya tienes un juego en curso. ¡Termina ese primero!", m, rcanal);
...
conn.reply(m.chat, "⚠️ *Solo puedes enviar una letra a la vez.*", m, rcanal);
}};
