...
const handler = async (m, {conn, usedPrefix, command}) => {
  ...
  if (!who) {
  conn.reply(m.chat, `*☁️ Etiqueta al usuario.*`, m, rcanal)
  return;
    };
...
return;
  ...
  if (users.coin < rob) return conn.reply(m.chat, `😔 @${who.split`@`[0]} Tiene menos de *${ro} coins*\nNo robes a un pobre :v`, m, {mentions: [who]});
  global.db.data.users[m.sender].coin += rob;
  global.db.data.users[who].coin -= rob;
  conn.reply(m.chat, `*☁️ Robastes ${rob} coins a @${who.split`@`[0]}*`, m, {mentions: [who]});
