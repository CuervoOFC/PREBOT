...
*📌${mssg.example}:* _${usedPrefix+command} normal_`
  ...
  let id = m.chat
    if (id in conn.math) return conn.reply(m.chat, `⚠️ ${mssg.mathOn}`, conn.math[id][0], m, rcanal)
    let math = genMath(mode)
    conn.math[id] = [
        await conn.reply(m.chat, `▢ CUANTO ES *${math.str}*=\n\n*${mssg.time}:* _${(math.time / 1000).toFixed(2)}_ *${mssg.second}*\n\n🎁 ${mssg.reward} : ${math.bonus} 🪙`, m, rcanal),
        math, 4,
        setTimeout(() => {
            if (conn.math[id]) conn.reply(m.chat, `⏳ ${mssg.timeOff} *${math.result}*`, conn.math[id][0], m, rcanal)
      delete conn.math[id]
        }, math.time)
    ]
}
