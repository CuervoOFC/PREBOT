...
*${usedPrefix + command}* 100`.trim()
    let img 'https://i.ibb.co/wF8GPMcB/file.jpg'

    if (!args[0]) throw await conn.sendMessage(m.chat, {
      image: { url: img },
      caption: fa,
      footer: author,
      buttons: [
        {
          buttonId: `/slot 100`,
          buttonText: {
            displayText: 'SLOT 100',
          },
        },
        {
          buttonId: `/slot ${user.coin}`,
          buttonText: {
            displayText: 'SLOT ALL',
          },
        },
      ],
      viewOnce: true,
   ...
   if (maxap < apuesta) return conn.reply(m.chat, `🎰 ${mssg.betMax} *${maxap} 🪙*`, m, rcanal) 
