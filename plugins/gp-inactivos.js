...
let handler = async (m, { conn, text, args, groupMetadata }) => {
    ...
    if (args[0] === 'list') {
        if (total === 0) return conn.reply(m.chat, `*No hay mierdas en este grupo.*`, m, rcanal)
    ...
    if (args[0] === 'kick') {
        if (total === 0) return conn.reply(m.chat, `*No hay mierdas para patear en este grupo.*`, m, rcanal)
        ...
        return conn.reply(m.chat, `Eliminado con éxito *${total}* miembros inactivos del grupo.`, m, rcanal)
    }

    return conn.reply(m.chat, `Opción no válida. Utilice \`list\` para ver miembros inactivos o \`kick\` para eliminarlos.`, m, rcanal)
}
