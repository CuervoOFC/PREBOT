...
if (!('audios_bot' in chat))
chat.audios_bot = false
if (!('audios' in chat))
chat.audios = false
if (!('detect' in chat))
chat.detect = true
...
let chat = global.db.data.chats[m.chat]
let user = global.db.data.users[m.sender]
if (!['gp-unbanchat.js'].includes(name) && chat && chat.isBanned && !isROwner) return // Except this
if (name != 'gp-unbanchat.js' && name != 'owner-exec.js' && name != 'owner-exec2.js' && name != 'gp-delete.js' && chat?.isBanned && !isROwner) return 
...
if (name != 'gp-unbanchat.js' && chat?.isBanned)
return 
...
m.isCommand = true
let xp = 'diamond' in plugin ? parseInt(plugin.diamond) : 17 
if (xp > 200)
m.reply('chirrido -_-')
else
m.diamond += xp
if (!isPrems && plugin.coin && global.db.data.users[m.sender].coin < plugin.coin * 1) {
conn.reply(m.chat, `Se agotaron tus *Diamantes*`, m, null, rpl)
continue
}
let extra = {
match,
...
if (m.coin)
conn.reply(m.chat, `Utilizaste *${+m.coin}* `, m, null, rpl)
}
break
}}
} catch (e) {
